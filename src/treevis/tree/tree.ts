...
class NodeImpl implements INode {
    ...
    get children(): ReadonlyArray<any> {
        let fields = this.fields
            .filter(field => (field.kind === "node" || field.kind === "nodeArray") && field.value)
            // .filter(field => Array.isArray(field.value) ? field.value.length > 0 : field['fields'] && field['fields'].length > 0  )
