...
export class Tabs extends LiteGuiElement{
	private mode: any;

	current_tab: any;
	private list: HTMLElement;
	private tabs_root: HTMLElement;
	private tabs: {};
	private tabs_by_index: any[];
	private selected: null;
	onchange: any;
	private previous_tab: any;
	_timeout_mouseover: any;
	private plus_tab: any;
	...
	private dataset: any;
		...
		*/
		constructor( options?, legacy? ) {
			...
			var mode: any = this.mode = options.mode || "horizontal";
		...
		*/
		...
		selectTab(id, skip_events? ) {
			...
			var tabs: any = this.list.querySelectorAll("li.wtab");
		...
		detachTab(id, on_complete, on_close ) {
			...
			var w = 800;
			var h = 600;
			var tab_window : any = window.open("","","width="+w+", height="+h+", location=no, status=no, menubar=no, titlebar=no, fullscreen=yes");
			...
			var newtabs = new Tabs(null, this.options );
