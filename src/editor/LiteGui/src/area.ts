...
export class Area {
		...
		sections: any[];
	onresize: any;
	private splitbar: any;
	private size: any;
	private direction: any;
	private dynamic_section: any;
		...
		*/
		constructor( options?, legacy? ) {
			...
			if( (options && options.constructor === String) || legacy )
			{
				...
				options = legacy || {};
				options.id = id;
				console.warn("Area legacy parameter, use options as first parameter instead of id.");
			}
		...
		split(direction, sizes, editable ) {
			...
			//create areas
			var area1 = new Area({ content_id: this.content.id });
		...
		moveSplit(delta) {
			...
			if(this.direction == "horizontal")
			{
				...
				{
					var size: any = (area1.root.offsetWidth - delta);
					if(size < min_size)
						size = min_size;
			...
			else if(this.direction == "vertical")
			{
				if (this.dynamic_section == area1)
				{
					var size: any = (area2.root.offsetHeight - delta);
					if(size < min_size)
						size = min_size;
	...
	export class Split {
		...
		* @constructor
		*/
		constructor( sections, options, legacy ) {
			options = options || {};
