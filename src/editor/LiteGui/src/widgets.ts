...
export class Button extends LiteGuiElement {
...
export class SearchBox extends LiteGuiElement  {
...
export class ContextMenu  extends LiteGuiElement  {
	private parentMenu: any;
	lock: any;
	current_submenu: any;
	...
	*/
	constructor( values, options : any ) {
		super()
		options = options || {};
...
export class Checkbox extends LiteGuiElement  {
	private element: any;
	private value: any;
	private setValue: (v) => void;
	constructor(value, on_change) {
		super();
		var that = this;
		this.value = value;
...
export class List extends LiteGuiElement  {
...
export class Slider extends LiteGuiElement  {
    _options: {
		min?: number;
		max?: number;
	};
	...
	*/
	constructor(value, options) {
		...
		function setFromX(x)
		{
			...
			var norm = x / width;
			// @ts-ignore
			var min = this._options.min || 0.0;
			// @ts-ignore
			var max = this._options.max || 1.0;
			var range = max - min;
		...
		root.addEventListener("mousedown", function(e) {
			...
			if(e.offsetX) { mouseX = e.offsetX; mouseY = e.offsetY; }
				...
				if(e.layerX) { mouseX = e.layerX; mouseY = e.layerY; }
			}
			setFromX(mouseX);
...
export class ComplexList extends LiteGuiElement  {
    onItemToggled: any;
	onItemRemoved: any;
	onItemSelected: any;
	private item_code: any;
