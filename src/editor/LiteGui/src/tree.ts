...
import {createLitebox} from "./widgets";
import {LiteGuiElement} from "./liteGuiElement";


export class Tree extends LiteGuiElement {
	private _skip_scroll: any;
	private indent_offset: number;
	private collapsed_depth: any;
	onMoveItem: any;
	onDropItem: any;
	...
	onBackgroundClicked: any;
		...
		*/
		...
		_isNodeChildrenVisible(id ) {
			...
			//check listbox
			var listbox: any = node.querySelector(".listbox");
			if(!listbox)
				return true;
		...
		createTreeItem(data, options, level ) {
			...
			var root : any = document.createElement("li");
				...
				if(!name || str.indexOf( name.toLowerCase() ) != -1)
				{
					...
					if(indent)
					{
						if(name)
							...
							indent.style.paddingLeft = ( (parseInt(childNode.dataset["level"]) + this.indent_offset) * Tree.INDENT) + "px";
					}
				}
				else
				{
...
//updates the widget to collapse
		_updateListBox( node, options?, current_level? ) {
			...
			if(!node.listbox)
			{
				...
				var box = createLitebox(true, function(e) {
