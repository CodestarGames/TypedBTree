...
export class Tree {
  ...
  build(obj) : any {
    ...
    this.cnt++;
    if (obj[self.childNodeName] && Object.entries(obj[self.childNodeName]).length > 0) {
      walk(obj[self.childNodeName], index);
    }

    function walk(objs, parent) {
      const children = [];
      if(Array.isArray(parent.node.children) === false)
        objs = [objs];
  ...
  removeIndex(index) {
    ...
    function del(index) {
      ...
      if (index[self.childNodeName] && Object.entries(index[self.childNodeName]).length > 0) {
        if(Array.isArray(index[self.childNodeName]) === false)
          index[self.childNodeName] = [index[self.childNodeName]];
  ...
  remove(id) {
    ...
    if(Array.isArray(parentNode[this.childNodeName]))
  ...
  insert(obj, parentId, i) {
    ...
    if(Array.isArray(parentNode[this.childNodeName]))
      ...
      parentNode[this.childNodeName] = Object.assign(parentNode[this.childNodeName], obj);
